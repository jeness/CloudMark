...
conf = SparkConf().setAppName("myproject").setMaster("local[*]")
...
rdd = sc.parallelize(input_data[0])
print(rdd.glom().collect())

resultRDD = rdd.flatMap(match_images)
...
#     print(inliers[index[i]])
    ...
    # data_name = data_list[index].replace('\\','').split('.')[0]
